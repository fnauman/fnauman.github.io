...
There was no single systematic validation layer that could answer this across the migration estate. Engineers could spot-check a table, run a `COUNT(*)`, or compare a date range manually, but there was no historical record and no dashboard the team could trust.
...
- **The first automated runs surfaced concrete row-count mismatches that manual spot checks had not made visible.** These were real data quality defects, not expected discrepancies.
...
| Largest table profiled | Very large event table |

...
A critical daily analytics pipeline had been running for years. Every morning, it recomputed several downstream tables from very large source data. The job was expensive enough that optimization mattered, but trusted enough that any redesign needed strong correctness guarantees.
...
|---------|---------------|------------------------|---------|
| Mid-scale | 85 seconds | 17 seconds | **5x** |
| Large-scale | 360 seconds | 30 seconds | **12x** |
...
| Shared-Incremental + `MERGE` | Zero structural diffs in completed validation runs | **Roughly 4-6x faster than baseline** |
| Full recompute | Correct | Baseline |

In production terms, that meant:

- daily runtime dropped by a large multiple
- compute cost fell materially
- the one-time seed cost paid back quickly
